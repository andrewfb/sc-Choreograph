...
class Movie {
  public:
	Movie( const NetflixMovie &info, Vec2f pos, Vec2f size )
		: mInfo( info ), mPos( pos ), mSize( size ), mAlpha( 1 ), mIconAlpha( 0 ), mTitleOffset( sTitleOffset )
	{
		TextLayout layout;
		layout.setColor( ColorA( 0.2f, 0.2f, 0.2f, 1 ) );
		layout.setFont( Font( "Springsteel Serif", 16 ) );
	...
	void draw() {
		...
		gl::color( ColorA( 1, 1, 1, mAlpha ) );		
		//gl::draw( mTitleTexture, Vec2f( floor( mPos.x + mTitleOffset.x ), floor( mPos.y + mTitleOffset.y ) ) );
		sTitleFont->drawString( mInfo.mTitle, mPos + mTitleOffset );
	}
...
void NetflixRssApp::setup()
{
	...
	sTitleFont = gl::TextureFont::create( Font( "Springsteel Serif", 16 ) );
	...
	Movie::sTitleOffset = Vec2f( 4, 2 );
	Movie::sExpandedTitleOffset = Vec2f( 68, 2 );
...
console() << parser.getMovies()[m] << std::endl;
		...
		initialFadeTln->append( &mMovies.back().mAlpha, 0.0f, 1.0f, 0.5f )->delay( -0.46f );
	}
